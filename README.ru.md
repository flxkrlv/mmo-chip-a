...
### Die Viewer — операции с ячейками

| Клавиши | Действие |
|---|---|
| `Ctrl+C` / `⌘C` (на выделенной ячейке) | Копировать ячейку (cellTypeId + ориентация) |
| `Ctrl+V` / `⌘V` | Вставить ячейку в позицию курсора |
| `Shift+U` | Make Unique — отвязать ячейку от типа, создать приватную копию слоёв |
| ПКМ на ячейке | Контекстное меню: Copy Cell / Make Unique |
| ПКМ (непустой буфер) | Контекстное меню: Paste Cell в место клика |
...
Инструменты для работы с инстансами ячеек на die viewer. Решают проблему множества одинаковых ячеек, когда одной из них нужны другие параметры.

### Copy / Paste

Выделите ячейку на die (клик по контуру) и нажмите `Ctrl+C` / `⌘C` — скопируется cellTypeId + ориентация (поворот, отражение). `Ctrl+V` / `⌘V` вставляет новый инстанс в позицию курсора с тем же типом и ориентацией.

Вставка также доступна из контекстного меню (ПКМ) когда буфер не пуст — новый инстанс создаётся в точке клика.

### Make Unique (`Shift+U`)

Отвязывает ячейку от общего cellType. Создаётся глубокая копия слоёв с новым ID, и только эта ячейка ссылается на неё. После Make Unique:

- Ячейку можно открыть в Cell RE и редактировать слои независимо
- Остальные инстансы оригинального типа не меняются
- Extraction автоматически перезапускается для нового приватного типа
- Undo поддерживается (один batch-шаг)

Вызов: ПКМ на ячейке → "Make Unique", или `Shift+U` на выделенной ячейке.

### Cell Relationship Overlay

Когда ячейка или cellType выбраны на die viewer (клик на канвасе или в Outline Tree), все ячейки того же типа подсвечиваются циановым halo. Остальные ячейки затемняются до 10% прозрачности — группа видна сразу.

- Включение: чекбокс **CELL REL** в правой панели (секция overlay)
- **Inspector** и **DeviceInspector** показывают строку "Relationship: Linked (×N)" или "Unique"
- Клик по группе cellType в Outline Tree подсвечивает все её инстансы
