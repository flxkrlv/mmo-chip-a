/**
 * analogDevices.ts — REFERENCE COPY (not used in production)
 *
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  Moved from frontend/src/lib/extraction/analogDevices.ts        ║
 * ║  Kept as reference — NOT imported anywhere in the app.          ║
 * ║  Replaced by simpleAnalog.ts (marker-based + well-based).       ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 * Notable algorithms in this file that exist ONLY here
 * (not replicated in simpleAnalog.ts):
 *
 *   bodyCenterline() / bodyCenterlineLength() / bodyAvgWidth()
 *     — Find the medial axis of a resistor body (PCA-based edge pairing)
 *     — Meander/serpentine detection
 *
 *   detectMultiplier()
 *     — Heuristic: find repeated identical structures with equal spacing
 *     — Centroids → common gap → count
 *
 *   polygonPrincipalAngle()
 *     — PCA-based orientation of a polygon
 *
 *   countFingers()
 *     — Count poly gates crossing a diffusion (vs simpleAnalog's gates.length)
 *
 *   computeMOSParams() / computeBJTParams() / computeResistorParams()
 *   computeCapacitorParams() / computeDiodeParams()
 *     — Bbox-based geometry parameter computation
 *
 *   detectBJT() / detectResistors() / detectCapacitors() / detectDiodes() / detectJFET()
 *     — Pure auto-detection heuristics (inference, no markers)
 *
 *   DetectionContext / DeviceMatch
 *     — Types for the Phase 1 auto-detection pipeline
 *
 * The functions above are NOT guaranteed to compile with current imports
 * (they reference ../geometry, ./cell, shared), but serve as algorithm
 * reference if auto-detection is revisited.
 */
