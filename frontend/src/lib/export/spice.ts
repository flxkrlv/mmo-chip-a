...
function terminalString(
  ...
  switch (kind) {
    ...
    case "bjt_pnp": {
      ...
      const base = `${tn("C")} ${tn("B")} ${tn("E")}`;
      // Spectre: add 4th terminal (substrate) tied to global ground 0
      return dialect === "spectre" ? `${base} 0` : base;
    }
    case "jfet_n":
    case "jfet_p":
...
function guessModelName(d: AnalogDevice): string {
  ...
  return kindNames[d.kind] ?? "unknown";
}
...
function deviceLine(
  ...
  const termStr = terminalString(d.kind, d.terminals, netLookup, vdd, gnd, dialect);
  const modelName = d.modelName ?? guessModelName(d);
...
function generateSpectre(
  ...
  const portNets = new Set<string>();
  for (const d of devices) {
    const termStr = terminalString(d.kind, d.terminals, netLookup, vdd, gnd, "spectre");
    for (const term of termStr.split(" ")) {
      const sanitized = term.trim();
      if (sanitized && sanitized !== "0" && sanitized !== vdd && sanitized !== gnd) {
        portNets.add(sanitized);
