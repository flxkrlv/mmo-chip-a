...
import type { TerminalSnapTarget, WireSnap } from "./useWireTool";

const DRAFT_COLOR = "#7fb2ff";
const POINT_RADIUS_PX = 4;

/** Preview endpoint of the rubber-band; `onNode` when snapped to an existing
 *  vertex, `onVia` when snapped to a via (ML or manual), `onTerminal` when
 *  snapped to a cell-instance terminal — each draws a different halo.
 *  `elbow` is the corner of the orthogonal L-route used when connecting into
 *  a vertex. */
...
export function WireDraftOverlay({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // an existing vertex (solid halo), a virtual vertex that will split a
      // wire body (dashed + plus), or a cell terminal (orange halo).
      if (points.length === 0) {
        ...
        if (snap) {
          ...
          } else if (snap.virtual && !snap.via) {
            drawVirtualVertex(ctx, s.x, s.y, r);
          } else {
            drawSnapHalo(ctx, s.x, s.y, r);
          }
        }
        return;
      }
      ...
      const orangeRing = (s: { x: number; y: number }) =>
